...
const Relatorios = ({ user }) => {
  ...
  // Cores atualizadas (o primeiro agora é o nosso Dourado)
  ...
  const renderTabelaServicos = (barbeiroNome, data) => {
    ...
    return (
      <div className="space-y-3">
        <h3 className={`font-bold text-sm flex items-center gap-2 p-2.5 rounded-lg ${barbeiroNome === 'Miguel' ? 'bg-[#DEAE60]/10 text-yellow-800' : 'bg-green-50 text-green-800'}`}>
            ...
            filtrados.map((s, i) => (
              <div key={i} className="flex items-center justify-between p-3 border border-gray-100 bg-white/50 rounded-lg hover:bg-gray-50 transition-colors">
                <div>
                  ...
                  <p className="text-[10px] text-gray-500 font-medium uppercase tracking-widest mt-0.5">Quantidade: {barbeiroNome === 'Miguel' ? s.miguel_qty : s.jhonatas_qty}</p>
                </div>
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pt-8 sm:pt-4">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-2">
        <div>
          <h1 className="text-3xl font-black text-white uppercase tracking-tighter drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">Relatórios Profissionais</h1>
      ...
      {/* FILTROS (BRANCO COM VIDRO FOSCO) */}
          ...
          <CardContent className="pt-6 flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <Select value={periodo} onValueChange={setPeriodo}>
                <SelectTrigger className="border-none shadow-none p-0 h-auto font-black text-gray-900 text-lg focus:ring-0 bg-transparent"><SelectValue /></SelectTrigger>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={servicosMaisVendidos} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
                      ...
                      {(barber === 'Geral' || barber === 'Miguel') && (
                        <Bar name="Miguel" dataKey="miguel_qty" fill="#DEAE60" radius={[4, 4, 0, 0]} barSize={barber === 'Geral' ? 30 : 50} />
                      )}
          ...
          <Card className="bg-white/90 backdrop-blur-md border-white/40 shadow-xl">
            <CardHeader className="border-b border-gray-200/50 bg-white/50">
              <CardTitle className="flex items-center gap-2 text-lg font-bold uppercase tracking-tight text-gray-900">
                <TrendingUp className="h-5 w-5 text-green-600" /> Evolução de Faturamento {periodo === 'hoje' ? '(Por Hora)' : ''}
              </CardTitle>
            </CardHeader>
            <CardContent className="pt-6">
                ...
                <div className="space-y-4">
                  {byPayment.map((p, i) => (
                    <div key={i} className="flex items-center justify-between p-3 bg-white border border-gray-100 shadow-sm rounded-xl">
                      ...
                      <div className="text-right">
                        <p className="font-black text-sm text-gray-900">R$ {p.valor.toLocaleString('pt-BR', {minimumFractionDigits: 2})}</p>
                  ...
                  <div className="pt-4 mt-2 border-t border-dashed border-gray-300 space-y-3">
                    <div className="flex items-center justify-between p-4 bg-[#DEAE60]/10 rounded-xl border border-[#DEAE60]/30 shadow-inner">
                      ...
                      <div className="text-right">
                        <p className="font-black text-xl text-yellow-700">R$ {totalReceita.toLocaleString('pt-BR', {minimumFractionDigits: 2})}</p>
                      </div>
                    </div>
                    ...
                    {mostrarComissao && (
                      <div className="flex items-center justify-between p-4 bg-green-50 rounded-xl border border-green-200 shadow-inner">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-xs font-black text-green-900 uppercase tracking-widest">COMISSÃO JHONATAS</span>
                        </div>
                        <div className="text-right">
                          <p className="font-black text-xl text-green-700">R$ {comissaoJhonatas.toLocaleString('pt-BR', {minimumFractionDigits: 2})}</p>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {produtosVendidos.map((p, i) => (
                        <tr key={i} className="hover:bg-white transition-colors bg-white/40">
                          ...
                          <td className="px-6 py-4 text-right font-black text-green-600 text-base whitespace-nowrap">
                    ...
                    <tfoot className="bg-[#DEAE60]/10 border-t-2 border-[#DEAE60]/30">
                      <tr>
                        <td colSpan="3" className="px-6 py-5 text-right font-black text-yellow-900 uppercase tracking-widest text-xs">TOTAL EM PRODUTOS:</td>
                        <td className="px-6 py-5 text-right font-black text-yellow-700 text-xl whitespace-nowrap">
                ...
                {frequenciaClientes.map((c, i) => (
                  <div key={i} className="flex items-center justify-between p-4 bg-white border border-gray-100 rounded-xl hover:bg-gray-50 transition-colors shadow-sm">
                    <div className="flex items-center gap-4">
                      <div className={`h-12 w-12 rounded-full flex items-center justify-center font-black text-lg shadow-inner ${i < 3 ? 'bg-[#DEAE60] text-neutral-950 shadow-black/20' : 'bg-gray-100 text-gray-600 border border-gray-200'}`}>
                        {i + 1}º
                      </div>
                      <div>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xl font-black text-gray-900 whitespace-nowrap">R$ {c.gasto.toLocaleString('pt-BR', {minimumFractionDigits: 2})}</p>
                    </div>
                  </div>
                ))}
