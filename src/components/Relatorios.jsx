  ...
  useEffect(() => {
    const fetchNomes = async () => {
      try {
        ...
        if (resNome1.ok) {
          const data1 = await resNome1.json();
          setBarberOneName(data1.valor || 'Fabrício');
        }
        
        if (resNome2.ok) {
          const data2 = await resNome2.json();
          const nomeSalvo = data2.valor;
          // TRAVA: Força Gabriel caso seja Jhonatas ou vazio
          setBarberTwoName((nomeSalvo === 'Jhonatas' || !nomeSalvo) ? 'Gabriel' : nomeSalvo);
        }
        
        if (resNome3.ok) {
          const data3 = await resNome3.json();
        ...
        if (response.ok) {
          ...
          if (Array.isArray(data.top_clients)) {
            const clientesOrdenados = data.top_clients
              .map(c => ({ nome: c.name, visitas: c.visits, gasto: c.spent }))
  ...
  const renderTabelaServicos = (barbeiroKey, nomeExibicao, data) => {
    ...
    return (
      <div className="space-y-3">
        <h3 className={`font-bold text-sm flex items-center gap-2 p-2.5 rounded-lg ${getBgColor()}`}>
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pt-8 sm:pt-4">
          ...
          <CardContent className="pt-6 flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <Select value={periodo} onValueChange={setPeriodo}>
                <SelectTrigger className="border-none shadow-none p-0 h-auto font-bold text-gray-900 text-lg focus:ring-0 bg-transparent">
                  <SelectValue />
                </SelectTrigger>
      ...
      <Tabs defaultValue="receita" className="space-y-6">
        ...
        <TabsContent value="servicos" className="space-y-6">
          ...
          <Card className="bg-white/90 backdrop-blur-md border-white/40 shadow-xl">
            <CardHeader className="border-b border-gray-200/50 bg-white/50">
              <CardTitle className="text-lg font-bold uppercase tracking-tight text-gray-900">Detalhe Numérico</CardTitle>
            </CardHeader>
            <CardContent className="pt-6">
